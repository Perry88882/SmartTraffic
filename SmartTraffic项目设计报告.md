...
| 模块 | 负责人 | 核心工作 | 难度 |
|------|--------|----------|------|
| 后端架构 + WebSocket + 数据库 | **成员 A** | Flask 工厂、REST API、SocketIO 实时推送、SQLite 持久化 | ★★★★ |
| 数据管道（全链路） | **成员 B** | scapy 抓包、包解析、流重组、特征提取、推理分类 | ★★★★ |
| 前端仪表盘 | **成员 C** | React 组件、ECharts 图表、SocketIO 客户端、深色主题 | ★★★★ |
| 测试 + 文档 + 集成部署 | **成员 D** | 单元测试、联调测试、项目文档、演示 PPT、Git 管理 | ★★★ |
